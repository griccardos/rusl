///TODO:
///
/// 2. horizontal scrollbar of list does not work
/// 4. add more options
use crate::manager::{Manager, SearchResult};
...
pub fn run_slint() {
    ...
    //channels to communicate between gui and backend
    //  let (file_sender_channel, file_receiver_channel) = std::sync::mpsc::channel();
    //  let file_sender_quit = file_sender_channel.clone();
    //  let file_sender_search = file_sender_channel.clone();
...
/*fn file_receiver(
    ...
    loop {
        ...
        match message {
            Message::StartSearch(sch) => {
                names_done = false;
                contents_done = false;
                tot_elapsed = Duration::from_secs(0);
                set_data(weak.clone(), vec![], Duration::default(), true);
                search = sch
            }
            Message::ContentFiles(files, number, elapsed) => {
                eprintln!("Found content{}", files.len());
                if number >= latest_number {
                    latest_number = number;
                    //only update if new update (old updates are discarded)
                    contents_done = true;
                    contents = HashMap::from_iter(
                        files
                            .iter()
                            .map(|x| (x.path.to_owned(), x.to_owned()))
                            .collect::<Vec<(String, FileInfo)>>(),
                    );
                    tot_elapsed += elapsed;
                }
            }
            Message::Files(files, number, elapsed) => {
        ...
        //check if need to set data
        let files = match (&search.search_type, names_done, contents_done) {
            (SearchType::FileNames, true, _) => {
                Some(names.values().cloned().collect::<Vec<FileInfo>>())
            }
            (SearchType::FileContents, _, true) => {
                Some(contents.values().cloned().collect::<Vec<FileInfo>>())
            }
            (SearchType::FileNamesAndContents, true, true) => {
                let common: HashSet<String> = names
                    .keys()
                    .cloned()
                    .collect::<HashSet<String>>()
                    .intersection(&contents.keys().cloned().collect::<HashSet<String>>())
                    .cloned()
                    .collect();
                //only select contents where we also have file names
                let files: Vec<FileInfo> = contents
                    .iter()
                    .filter(|x| common.contains(x.0))
                    .map(|x| x.1.to_owned())
                    .collect();
        ...
        if let Some(files) = files {
            ...
            res.clear();
            res.extend_from_slice(&files);
            let sort_type = sort_type.lock().unwrap();
            do_sort(&mut res, &*sort_type);

            set_data(
                weak.clone(),
                res.iter().map(|x| x.to_owned()).collect(),
                tot_elapsed,
                false,
            )
        }
...
fn set_data(weak: Weak<MainWindow>, files: Vec<FileInfo>, elapsed: Duration, reset: bool) {
    weak.upgrade_in_event_loop(move |weak| {
